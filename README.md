...
A catalogue of Thymer plugins by [akaready](https://github.com/akaready).

Every plugin below lives in its own repository and can be installed straight from the
**Plugins Manager** — open its **Discover** tab, or use **Install Plugin** with the repo URL.

To see these in Discover, add this file as a community source in
Plugins Manager → Settings → community repositories (one URL per line):

```
https://raw.githubusercontent.com/akaready/thymer-plugin-index/main/README.md
```

> This file is generated from the plugin manifests and republished automatically on every
> release, so it never drifts out of step with the plugins themselves.
